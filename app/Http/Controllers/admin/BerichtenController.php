...
class BerichtenController extends Controller
{
	...
	public function index() {
		...
		$Messages = DB::table('tbl_emp_messages')->orderBy('created_at','DESC')/*->groupBy(array('emp_id'))->where(array( 'admin_view' => 0))*/->get();
		//echo count($Project_Messages); die;
		//$project_ID = $id;
		return View('admin.Berichten.messages',compact('Messages'));
	...
	public function getconversation (Request $request) {
		...
		$project_id = $request->project_id;
		$messagelist = DB::table('tbl_emp_messages')->where('tbl_emp_messages.project_id',$project_id)->orWhere('tbl_emp_messages.client_id',$contact_id)->orderBy('msg_id', 'ASC')->get();
	...
	public function message_details($id) {
		...
		$All_Messages = DB::table('tbl_emp_messages')->orderBy('created_at','DESC')->where('project_id','=',$id)->get();
		if ($All_Messages) {
		DB:: table( 'tbl_emp_messages' )->where('project_id','=',$id) -> update( array( 'admin_view' => 1 ));		
		}
	...
	public function NewMessage () {
		...
		$data = array (	'emp_id' => $_POST['client_id'],
		...
		if (!empty($_POST['message'])) {
			...
			if ($_POST['redirect'] == 1) {
			return redirect('admin/EMPBerichten');
			} else {
			return redirect('admin/view_EMPBerichten/'.$_POST['msg_id']);
			}			
	...
	public function ProjectByContact () {
		...
		$All_Messages = DB::table('tbl_emp_messages')->orderBy('created_at','DESC')->get();
		 //echo $All_Messages->message;
		//print_r($All_Messages);
		return View('admin.Berichten.project_messages',compact('All_Messages'));
	...
	public function AllMessage($id) {
		...
		return View('admin.Berichten.allmessages',compact('All_Messages','Get_Message'));
	...
	public function CheckedBy($id,$chkid) {
		...
		if ($Get_Message) {
		DB:: table( 'tbl_emp_messages' )->where('msg_id','=',$id) -> update( array('status' => 1, 'admin_view' => 1,'checked_by' => Auth::user()->id,'checked_at' => date('Y-m-d H:i:s',time())));		
		}
		...
		if ($chkid == 0) {
			...
			Session::flash('message', 'Bericht met succes gecontroleerd..!');
			return redirect('admin/view_EMPBerichten/'.$Get_Message->msg_id);
		}
	}
	...
	public function Destroy($id) {
		...
		$Get_Message = DB::table('tbl_emp_messages')->where('msg_id','=',$id)->delete();
	...
	public function loadData($id)
    {
        ...
        return view('admin.Berichten.allmessages_ajax',compact('All_Messages','Get_Message'));
    }
	...
	public function loadDataAjax(Request $request)    {
        ...
        if(!$Messages->isEmpty())
        {
			...
			foreach($Messages as $message) {				
							...
							@$GetEmpName = DB::table('tblemployee')->where('id',@$message->sender_id)->first();
    ...
    width: 1190px;">
                            ...
                            <input type="hidden" id="msg_'.$message->msg_id.'" value="'.$message->message.'" />
                                   <input type="hidden" id="client_'.$message->msg_id.'" value="'.$message->emp_id.'" />
                   ...
                   <input type="hidden" id="ClientName'.$message->msg_id.'" value="'.@$GetEmpName->Firstname.' '.@$GetEmpName->Lastname.'" />';
				            if (!$GetEmpName) {
                            $output .= '<div class="sender" style="float:'.$float.';margin-top: 12px; margin-right:-10px; font-weight:bold">'.Auth::user()->name.'</div>';
                                
                                } else {
                                $output .= '<div class="sender" style="float:'.$float.';margin-top: 12px;margin-left: -32px;font-weight:bold">'. $GetEmpName->Firstname.'</div>';
                                 } 
								...
								Beantwoord</a>';
								} if (Auth::user()->group == 0 && $message->sender_id != $message->emp_id) {
