...
//------------------------------------
    ...
    function calculateSumAtt() {
        ...
        $(".tx").each(function() {
            ...
            if(!isNaN(this.value) && this.value.length!=0) {
                ...
                sum += parseFloat(this.value) ? parseFloat(this.value) :0 ;
        ...
        $("#total-timecard__").html(sum.toFixed(2));
    ...
    function GETRATES(id){
        ...
        var Employee_Id = $('select#Employee_ID_'+id).val();
        var project_id = $('#project').val();
        ...
        $.get('<?php echo URL:: to( 'admin/AjaxemployeesCost'); ?>?Employee_Id=' + Employee_Id+'&project_id='+project_id,function(data) {
            ...
            $('#rate_cost_'+id).val(data.Cost);

            // alert(data.Cost);

            $('#rate_'+id).val(data.Rate);
    ...
    <div class="col-md-12">
        ...
        @if ($EMPDATA)
            ...
            <div class="block">
                ...
                <div class="header">
                    {!! Form::open(['url' => 'admin/AddweekcardMultiple', 'id' => 'timecard']) !!}
                ...
                <div class="content">
                    ...
                    <div class="form-row">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <select class="select2" style="width: 100%;" tabindex="-1"
                                                name="Employee_Id[]" onchange="GETRATES(<?php echo $IDS_Count ?>)" id="Employee_ID_{{$IDS_Count }}">

                                            <?php foreach ($AllEmployees as $Employee) {?>
                                            @if ($Employee->id == $item->employee_id)
                                                <option selected
                                                        value="<?= $Employee->id . '_' . $Employee->Employmentagency_Id ?>">
                                    ...
                                    {{-- <td><input class="form-control txt" type="text" style="padding:2px;"
                                            onchange="this.value=fmtFloat(this.value.replace(',','.')); " value=""
                                            name="hours_1"></td>
                                    ...
                                    <td><input class="form-control txt" type="text" style="padding:2px;"
                                            onchange="this.value=fmtFloat(this.value.replace(',','.')); " value=""
                                            name="hours_5"></td> --}}
                                    @for ($i = 0; $i < count($item->WEEK_ATTEND); $i++)


                                        <td><input class="form-control tx" type="text" style="padding:2px;"
                                                   onchange="this.value=fmtFloat(this.value.replace(',','.')); " value=" {{ $item->WEEK_ATTEND[$i]['Attendence'] ? $item->WEEK_ATTEND[$i]['Attendence']->total_time :'' }}"
                                                   name="hours_{{$i}}[]"></td>
                                    @endfor
                                    <td><input class="form-control tx" type="text" style="padding:2px;"
                                               onchange="this.value=fmtFloat(this.value.replace(',','.')); " value=""
                                    ...
                                    <td>
                                        ...
                                        <div id="total-timecard__" class="total">{{$item->TOTAL_HOURS ? $item->TOTAL_HOURS : 0.00 }}</div>
                                    </td>

                                    <td><input type="text" id="rate_{{$IDS_Count }}" value="{{$item->RATE_COST['Rate']}}" class="form-control" style="padding:2px;"
                                               name="rate[]" /></td>

                                    <td><input type="text" id="rate_cost_{{$IDS_Count }}" value="{{$item->RATE_COST['Cost']}}" class="form-control" style="padding:2px;"
                                               name="rate_cost[]" /></td>

                                    <td><input type="checkbox" name="YESSS" value="1" checked></td>
                                    {{--                                        <input type="checkbox" name="Checked[]" value="1" >--}}

                                    <td>
                                        <select class="select2" style="width: 100%;" name="notes[]"
                                                id="drop_notes">
                                            <option value="">Opmerkingen kiezen</option>
                                            <?php
                                            if (!empty($Comments)) {
                                            ...
                                            <option value="<?= @$Comment->comments ?>"><?= @$Comment->comments ?>
                                            </option>
                                            <?php }}  ?>
                                        ...
                                        <input type="text" class="form-control"
                                               style="padding:2px;display:none; margin-top:5px;" name="note_other[]"
                                               id="note_other" />

                                    </td>

                                    {{-- <td id="buttoncol"><button class="btn btn-success" type="submit"><span
                        ...
                        <button class="btn btn-success" type="submit"><span
                                    class="icon-plus-sign"></span> Submit</button>
                        {!! Form::close() !!}
