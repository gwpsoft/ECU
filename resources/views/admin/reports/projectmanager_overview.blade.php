...
<script>  
		...
		$( "#search" ).click(function() {
			...
			document.location.href="<?=URL:: to( 'admin' )?>/report/projectmanageroverview/weeknumber/" + year + week;
 ...
 <title>Report</title>   
   ...
   <div class="block">
    ...
    for ($i=-2; $i<=2; $i++) {
        ...
        echo '<a href="' . URL:: to( 'admin' ) . '/report/projectmanageroverview' . '/weeknumber/' . $weeknumber . '"><span class="weeknr_' . $class . '" id="year">' . $weeknumber . '</span></a>';
                        ...
                        <table cellpadding="0" cellspacing="0" width="100%" class="table table-bordered table-striped sortable">
                            ...
                            <thead>
                                ...
                                <tr>
                                    ...
                                    <th width="10%">Weekstaat</th>

                                     <th width="20%">Project Manager</th>

                                     <th width="25%">Project</th>

                                    <th width="12%">Gewerkte uren</th>
                            ...
                            <?php if ($ProjectManagerOverView){  ?>

                             <tbody>

							<?php foreach ($ProjectManagerOverView as $Manager){ ?>                        

        <tr>

         <td>{{ @$Manager->Weekstate }}</td>

          <td>{{ @$Manager->ProjectManager }} </td>

            <td>{{ @$Manager->Name }}</td> 

             <td><?php echo($Manager->Hours > 0 ? '<span class="label label-success">'.$Manager->Hours.'</span>' : $Manager->Hours); ?></td> 

            <td>€ <?php echo($Manager->Rate > 0 ? '<span class="label label-success">'.$Manager->Rate.'</span>' : $Manager->Rate); ?></td>            

            <td>€ <?php echo($Manager->Cost > 0 ? '<span class="label label-success">'.$Manager->Cost.'</span>' : $Manager->Cost); ?></td>

            <td>€ <?php echo($Manager->Calculated > 0 ? '<span class="label label-success">'.$Manager->Calculated.'</span>' : $Manager->Calculated); ?></td>
