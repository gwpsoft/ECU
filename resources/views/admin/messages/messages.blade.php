...
$(document).ready(function(){
    $('#table').dataTable( {
	 ...
	 /* Datatables "aaSorting": [[ 0, "desc" ]]*/
                ...
                //     $("table.sortable").dataTable({"iDisplayLength": 10, "aLengthMenu": [10,25,50,100], "sPaginationType": "full_numbers", "aoColumns": [ { "bSortable": true }, null, null, null, null], "aaSorting": [[ 0, "desc" ]],});
                /* eof Datatables */
 ...
 <title>Lijst van Berichten</title>
   ...
   <div class="block">
                    ...
                    <div class="content">
                        ...
                        <table cellpadding="0" cellspacing="0" width="100%" id="table" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    <th width="8%">ID</th>
                                    <th width="15%">Contact Naam</th>
                            ...
                            @foreach($Messages as $key => $value)
							 ...
							 @$GetCustomerName = DB::table('tblcontact')->where('Id',@$value->client_id)->first();
							...
							@ $GetSenderID = DB::table('users')->where('contact_id',@$value->sender_id)->first();
							 ...
							 // print($GetSenderID->group);
							  ...
							  if (Auth::user()->group == 0 && $value->sender_id != $value->client_id) {
							  ...
							  ?>
                                <tr <?php echo $bold?>>
                                   ...
                                   <input type="hidden" id="client_<?php echo $value->msg_id?>" value="<?php echo $value->client_id?>" />
                                   <input type="hidden" id="project_<?php echo $value->msg_id?>" value="<?php echo $value->project_id?>" />
                                    ...
                                    <input type="hidden" id="ClientName<?php echo $value->msg_id?>" value="<?php echo  @$GetCustomerName->Firstname.' '.@$GetCustomerName->Lastname ; ?>" />
                                   </td>
                                   <td><?php echo  @$GetCustomerName->Firstname.' '.@$GetCustomerName->Lastname ; ?> </td>
                                    <td><?php echo  @$GetProjectName->Name ; ?>
                                    ...
                                    <td><?php echo  @$CheckedByName ?></td>
                                     ...
                                     <td>
<a href="<?php echo URL:: to( 'admin/view_Berichten',$value->msg_id); ?>" title="Alle Berichten" class="widget-icon"><span class="icon-comments"></span></a>
<?php if (Auth::user()->group == 0 && $value->sender_id != $value->client_id) {?>
<a href="#" onclick="Edit('<?php echo $value->msg_id?>')" title="Bewerk" class="widget-icon"><span class="icon-pencil"></span></a>
<a href="<?php echo URL:: to( 'admin/Del_Berichten',$value->msg_id ); ?>" title="Verwijderen" class="widget-icon" onclick="return confirm('Bent u zeker dat u dit wilt Werknemer verwijderen?');"><span class="icon-trash"></span></a>
<?php } else {
  if (empty($value->checked_by)) { // new
?>
 <a href="<?php echo URL:: to( 'admin/CheckedMessage/'.$value->msg_id,0); ?>" class="widget-icon" title="Bericht gezien"><span class="icon-eye-open"></span></a>
